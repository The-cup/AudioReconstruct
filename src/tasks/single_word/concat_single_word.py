...
INPUT_DIR = Path(r"D:\projects\python\AudioReconstruct\data\raw\SingleWord\WordSelected")
# 拼接后音频输出路径
OUTPUT_PATH = Path(r"D:\projects\python\AudioReconstruct\data\raw\SingleWord\merged_words.wav")
# 统一采样率（和你项目现有参数保持一致）
...
# ========== 低通滤波版专属配置 ==========
LOW_PASS_CUTOFF_HZ = 550        # 低通截止频率，过滤550Hz以上的高频内容
LOWPASS_OUTPUT_PATH = Path(r"D:\projects\python\AudioReconstruct\data\raw\merged_words_lowpass.wav")
...
def merge_lowpass_words_with_time_slots():
    """
    低通滤波版拼接：先将所有单词音频过滤550Hz以上内容，再按微标记规则拼接
    仅对单词发音滤波，微标记、静音段保持原样，兼容原有切分逻辑
    """
    # 1. 读取分层单词数据集，展平为有序列表
    word_dataset = load_word_audio_dataset(INPUT_DIR)
    word_order = []
    ...
    if not word_path_list:
        raise ValueError(f"输入目录 {INPUT_DIR} 下未找到有效音频文件")
    print(f"【低通滤波版】共待拼接 {len(word_path_list)} 条单词发音，截止频率 {LOW_PASS_CUTOFF_HZ}Hz")

    # 2. 预生成标记音频（标记不做滤波，保证检测有效性）
    start_marker = generate_tone_marker()
    end_marker = generate_tone_marker()

    # 3. 初始化最终音频 + 累计时间变量
    final_audio = AudioSegment.silent(duration=FRONT_SILENCE_MS, frame_rate=TARGET_SAMPLE_RATE)
    current_time_ms = FRONT_SILENCE_MS
    ...
    for idx, word_path in enumerate(word_path_list):
        ...
        # ========== 新增：550Hz低通滤波，仅处理单词主体 ==========
        word_audio = word_audio.low_pass_filter(LOW_PASS_CUTOFF_HZ)
        ...
        # 构造单单词单元：起始标记 + 0.2s静音 + 低通单词 + 0.2s静音 + 结束标记
        unit = start_marker
        unit += AudioSegment.silent(duration=MARKER_WORD_GAP_MS, frame_rate=TARGET_SAMPLE_RATE)
        unit += word_audio
        unit += AudioSegment.silent(duration=MARKER_WORD_GAP_MS, frame_rate=TARGET_SAMPLE_RATE)
    ...
    # 5. 导出结果 + 保存单词顺序
    LOWPASS_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    final_audio.export(str(LOWPASS_OUTPUT_PATH), format="wav")

    order_save_path = LOWPASS_OUTPUT_PATH.with_name("word_order_lowpass.txt")
    with open(order_save_path, "w", encoding="utf-8") as f:
        f.write("\n".join(word_order))

    print(f"\n【低通滤波版】拼接完成，总时长 {len(final_audio) / 1000:.2f}s")
    print(f"合并音频输出：{LOWPASS_OUTPUT_PATH}")
    print(f"单词顺序表输出：{order_save_path}")


if __name__ == "__main__":
    # merge_words_with_time_slots()
    merge_lowpass_words_with_time_slots()
